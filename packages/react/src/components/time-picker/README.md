...
- value/defaultValue는 Date|null, onChange는 Date입니다. 입력 Date의 로컬 날짜를 보존하며 시각을 바꿉니다. 비어 있을 때 기준일은 2000-01-01이고 기준 시각은 00:00입니다. 외부 값을 자동 수정하지 않습니다.
...
- variant=grid(default)/wheel. wheel은 가운데 스냅되는 native scroll 컬럼이며 사용자 스크롤이 끝나면 가운데 값을 선택합니다. scrollend 미지원 엔진은 클릭/키보드로 확정할 수 있습니다. 프로그램 스크롤과 키보드 위치 이동만으로 값이 바뀌지 않습니다. 별도 회전/부드러운 스크롤 애니메이션을 사용하지 않아 reduced-motion에서도 즉시 이동합니다.
