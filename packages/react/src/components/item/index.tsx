...
function ItemTitle({ children, className }: { children: ReactNode; className?: string }) {
  return <span className={cn('text-label text-[color:var(--ids-color-on-surface)]', className)}>{children}</span>;
}

function ItemDescription({ children, className }: { children: ReactNode; className?: string }) {
  return <span className={cn('text-caption text-[color:var(--ids-color-on-muted)]', className)}>{children}</span>;
}
